...
```SQL
SELECT НАЗВАНИЕ FROM ПОТРЕБИТЕЛЬ WHERE EXISTS (SELECT * FROM ПОСТАВЩИК, ДЕТАЛЬ, ЗАКАЗ WHERE АДРЕС IN ('Советский', 'Канавинский') AND ЗАКАЗ.ПОТРЕБИТЕЛЬ = ПОТРЕБИТЕЛЬ.ИДЕНТИФИКАТОР AND ЗАКАЗ.ДЕТАЛЬ = ДЕТАЛЬ.ИДЕНТИФИКАТОР AND ДЕТАЛЬ.`ЦЕНА, РУБ` > 6000);
```
EMPTY SET
...
```SQL
SELECT НАИМЕНОВАНИЕ FROM ДЕТАЛЬ WHERE EXISTS (SELECT * FROM ПОТРЕБИТЕЛЬ, ДЕТАЛЬ, ЗАКАЗ WHERE (SELECT COUNT(*) FROM ЗАКАЗ WHERE ЗАКАЗ.`КОЛ-ВО` = 1 AND )AND ЗАКАЗ.ПОТРЕБИТЕЛЬ = ПОТРЕБИТЕЛЬ.ИДЕНТИФИКАТОР AND ЗАКАЗ.ДЕТАЛЬ = ДЕТАЛЬ.ИДЕНТИФИКАТОР AND ДЕТАЛЬ.`ЦЕНА, РУБ` > 6000);
```

c) какие детали не заказывали потребители с размером скидки менее 5%;
```SQL
SELECT НАИМЕНОВАНИЕ FROM ДЕТАЛЬ WHERE EXISTS (SELECT * FROM ПОТРЕБИТЕЛЬ, ЗАКАЗ, ДЕТАЛЬ WHERE ПОТРЕБИТЕЛЬ.`СКИДКА, %` < 5 AND ЗАКАЗ.ПОТРЕБИТЕЛЬ = ПОТРЕБИТЕЛЬ.ИДЕНТИФИКАТОР AND ЗАКАЗ.ДЕТАЛЬ = ДЕТАЛЬ.ИДЕНТИФИКАТОР);
```

d) найти потребителя, заказывавшего все детали, не поставляемые поставщиками из Сормовского района.
```SQL
SELECT НАЗВАНИЕ FROM ПОТРЕБИТЕЛЬ WHERE EXISTS (SELECT * FROM ПОТРЕБИТЕЛЬ, ЗАКАЗ, ДЕТАЛЬ WHERE ПОТРЕБИТЕЛЬ.`СКИДКА, %` < 5 AND ЗАКАЗ.ПОТРЕБИТЕЛЬ = ПОТРЕБИТЕЛЬ.ИДЕНТИФИКАТОР AND ЗАКАЗ.ДЕТАЛЬ = ДЕТАЛЬ.ИДЕНТИФИКАТОР);
```
...
```SQL
SELECT DISTINCT ЗАКАЗ.ДАТА FROM ЗАКАЗ WHERE ЗАКАЗ.`СУММА, РУБ` > 10000;
```
